...
export default function Void({ className }: { className?: string }) {
  ...
  const messages = [
    ...
    },
    {
      ...
      subtitle:
        "You've reached the edge of the map. Turn back before the void consumes you.",
    },
    {
  ...
  return (
    <div
      className={`relative min-h-screen flex flex-col items-center justify-center bg-[#05050a] text-[#f0f0f3] overflow-hidden font-sans ${className || ""}`}
    >
      <style jsx global>{`
        @keyframes fadeIn {
          from {
      ...
      <main className="relative z-10 w-full max-w-[900px] px-6 py-12 md:py-16 flex flex-col items-center gap-8 md:gap-12 text-center">
        ...
        >
          ...
          <code className="text-[10px] md:text-xs text-[#6b6b7a] tracking-widest font-mono uppercase">
            // ERROR: SYSTEM_VOID_0x19A
          </code>
        </header>
        ...
        <section className="flex flex-col items-center gap-6 md:gap-8">
          ...
          <div
            className={`transition-all duration-500 transform ${isChanging ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"}`}
          >
            <h1 className="text-2xl md:text-5xl font-bold tracking-tight mb-4 flex flex-col items-center">
              <span className="text-white/80">
                {messages[titleIdx].title[0]}
              </span>
